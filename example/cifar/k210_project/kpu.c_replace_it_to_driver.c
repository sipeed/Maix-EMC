...
static void kpu_quantized_global_average_pool2d(const kpu_model_gap2d_layer_argument_t *arg, kpu_model_context_t *ctx)
{
...
static void dump(uint8_t* addr, uint32_t len)
{
    int i;
    printf("====addr=%x, len=%d====\r\n",(uint32_t)addr,len);
    for(i=0; i<len; i++)
...
{_D;
    ...
    const kpu_model_quant_param_t q = arg->quant_param;
_D;
printf("memin=%x, memout=%x\r\n",arg->main_mem_in_address, arg->main_mem_out_address);
printf("count=%ld\r\n",count);
//for(int i=0;i<64;i++)
//    printf("%x ",src[8*8*i]);
//printf("\r\n");
...
{_D;
    const float *src = (const float *)(ctx->main_buffer + arg->main_mem_in_address);_D;
    float *dest = (float *)(ctx->main_buffer + arg->main_mem_out_address);_D;
    uint32_t in_channels = arg->in_channels, out_channels = arg->out_channels, ic, oc;_D;
    float *weights = arg->weights, *bias = arg->weights + in_channels * out_channels;_D;
    uint32_t act = arg->act;
    ...
    switch(act){
    case KLA_LINEAR:
        printf("linear act, do no thing");
        break;
    case KLA_RELU:
        for (oc = 0; oc < out_channels; oc++)
        {
            dest[oc] = dest[oc]>0 ? dest[oc] : 0;
        }
        break;
    case KLA_RELU6:
    default:
...
static void kpu_tf_quant_flatten(const kpu_model_tf_flatten_layer_argument_t *arg, kpu_model_context_t *ctx)
{
    const uint8_t *src = (const uint8_t *)(ctx->main_buffer + arg->main_mem_in_address);
    uint8_t *dest = (uint8_t *)(ctx->main_buffer + arg->main_mem_out_address);
    kpu_model_shape_t in_shape = arg->shape;
...
{_D
    ...
    //dump(layer.kernel_load_cfg.data.para_start_addr, 32*3*3*3);
    layer.kernel_pool_type_cfg.data.bwsx_base_addr = (uintptr_t)(ctx->model_buffer + arg->bn_offset);
    //dump(layer.kernel_pool_type_cfg.data.bwsx_base_addr, 256);
    layer.kernel_calc_type_cfg.data.active_addr = (uintptr_t)(ctx->model_buffer + arg->act_offset);
    //dump(layer.kernel_calc_type_cfg.data.active_addr, 256);
_D
printf("memout=%x\r\n",arg->main_mem_out_address);
...
static const char *str_layer_type(uint32_t type)
{
    switch (type)
    {
        ...
        case KL_TENSORFLOW_FLATTEN:
            return "TFFlatten";
        case KL_QUANTIZED_TENSORFLOW_FLATTEN:
            return "QuantTFFlatten";
        case KL_K210_CONV:
            return "K210Conv";
...
_D
    switch (cnt_layer_header->type)
    {
        ...
        case KL_GLOBAL_AVERAGE_POOL2D:
            ...
            break;
        case KL_QUANTIZED_GLOBAL_AVERAGE_POOL2D:
            kpu_quantized_global_average_pool2d((const kpu_model_gap2d_layer_argument_t *)layer_body, ctx);
            break;
        case KL_QUANTIZED_MAX_POOL2D:
        ...
        case KL_QUANTIZE:
            ...
            break;
        case KL_DEQUANTIZE:
            _D;kpu_kmodel_dequantize((const kpu_model_dequantize_layer_argument_t *)layer_body, ctx);
            break;
        case KL_REQUANTIZE:
        ...
        case KL_QUANTIZED_CONCAT:
            ...
            break;
        case KL_FULLY_CONNECTED:
            _D;kpu_kmodel_fully_connected((const kpu_model_fully_connected_layer_argument_t *)layer_body, ctx);
            break;
        case KL_TENSORFLOW_FLATTEN:
            kpu_tf_flatten((const kpu_model_tf_flatten_layer_argument_t *)layer_body, ctx);
            break;
        case KL_K210_CONV:
            _D;kpu_conv((const kpu_model_conv_layer_argument_t *)layer_body, ctx);
            return 0;
        case KL_K210_ADD_PADDING:
        ...
        case KL_K210_UPLOAD:
            ...
            break;
		case KL_QUANTIZED_TENSORFLOW_FLATTEN:
			kpu_tf_quant_flatten((const kpu_model_tf_flatten_layer_argument_t *)layer_body, ctx);
			break;
        default:
